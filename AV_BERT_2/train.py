...
def train_model():
    ...
    # model_orig.load_state_dict(torch.load('best_avbert_l2.pth'))
    # pre_model = BertCls(num_classes)
    # pre_model.load_state_dict(model_orig.state_dict(), strict=False)
    model = BertLSTMBayesClassifier(num_classes, num_hiddens, lstm_num_layers, bert_model_name ,n_samples)
    ...
    plt.legend()
    plt.savefig('output_image/acc/avbayes21_acc.png')
    plt.clf()
    ...
    plt.title('Bert', pad=15, size=15)
    plt.legend()
    plt.savefig('output_image/loss/avbayes21_loss.png')
    print('image saved')
